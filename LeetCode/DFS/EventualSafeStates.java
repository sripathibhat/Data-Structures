...
class Solution {
    ...
    boolean dfs(int s, int graph[][], int dp[]) {
        // if we have already done dfs for s, return its state
        if (dp[s] != -1) {
            return dp[s] == 1;
        }
        // initialize to 0 to indicate it is visited.
        dp[s] = 0;
        for (int nei: graph[s]) {
            boolean state = dfs(nei, graph, dp);
            if (!state) {
                return false;
            }
        }
